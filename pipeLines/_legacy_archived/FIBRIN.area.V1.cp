...
LoadImages:[module_num:1|svn_version:\'Unknown\'|variable_revision_number:11|show_window:False|notes:\x5B\'1. Find Tissue area (green)\', \'2. Within Tissue area (green) Identify\x3A\', \' a. HE nuclei (blue)\', \' b. DAB area (yellow)\', \'3. Within DAB area (yellow) Identify\x3A\', \' a. DAB nuclei (red)\'\x5D]
    ...
    Text that these images have in common (case-sensitive):tile.tissue.bmp
    Position of this image in each group:2
    Extract metadata from where?:File name
    Regular expression that finds metadata in the file name:ENTROPY_(?P<NR>\x5B^.\x5D*)\\.(?P<STAIN>\x5B^.\x5D*).*\\.X(?P<X>\x5B0-9\x5D{1,4}).*\\.Y(?P<Y>\x5B0-9\x5D{1,4})
    Type the regular expression that finds metadata in the subfolder path:.*\x5B\\\\/\x5D(?P<Date>.*)\x5B\\\\/\x5D(?P<Run>.*)$
    Channel count:1
    Group the movie frames?:No
    Grouping method:Interleaved
    Number of channels per group:3
    Load the input as images or objects?:Images
    Name this loaded image:entropy_image
    Name this loaded object:Nuclei
    Retain outlines of loaded objects?:No
...
IdentifyPrimaryObjects:[module_num:3|svn_version:\'Unknown\'|variable_revision_number:8|show_window:False|notes:\x5B"Identify Tissue area\'s (green)"\x5D]
    Select the input image:entropy_image
...
UnmixColors:[module_num:4|svn_version:\'Unknown\'|variable_revision_number:2|show_window:False|notes:\x5B\'Split Fibrin and Collagen into separate channels\'\x5D]
    Stain count:2
    ...
    Image name:Collagen
    Stain:Custom
    Red absorbance:0.724924
    Green absorbance:0.646672
    Blue absorbance:0.23728

MaskImage:[module_num:5|svn_version:\'Unknown\'|variable_revision_number:3|show_window:False|notes:\x5B\'Only analyse DAB in the previously defined Tissue\'\x5D]
    Select the input image:Fibrin
    Name the output image:Tissue_Masked_MSB-channel
    Use objects or an image as a mask?:Objects
    Select object for mask:Tissue_object
    Select image for mask:Tissue_object_green
    Invert the mask?:No

IdentifyPrimaryObjects:[module_num:6|svn_version:\'Unknown\'|variable_revision_number:8|show_window:False|notes:\x5B"Identify DAB positive area\'s (yellow)"\x5D]
    Select the input image:Tissue_Masked_MSB-channel
...
MaskImage:[module_num:7|svn_version:\'Unknown\'|variable_revision_number:3|show_window:False|notes:\x5B\'Only analyse Nuclei (red) in the previously defined DAB tissue (yellow)\'\x5D]
    Select the input image:Fibrin
...
OverlayOutlines:[module_num:8|svn_version:\'Unknown\'|variable_revision_number:2|show_window:False|notes:\x5B\'The next outline is ontop of the previous one.\'\x5D]
...
SaveImages:[module_num:9|svn_version:\'Unknown\'|variable_revision_number:7|show_window:False|notes:\x5B\'Save outlined images\', \'These image are usually not used for further analysis and can be stored in lossy JPEG if wanted.\'\x5D]
...
MeasureImageAreaOccupied:[module_num:10|svn_version:\'Unknown\'|variable_revision_number:3|show_window:False|notes:\x5B\'Measure Tissue (green) and HE nuclei (blue) and DAB area (yellow) and red) area\'\x5D]
    Hidden:2
...
ExportToDatabase:[module_num:11|svn_version:\'Unknown\'|variable_revision_number:20|show_window:False|notes:\x5B\'Export findings to database for further analysis\'\x5D]
