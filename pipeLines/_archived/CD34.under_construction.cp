...
UnmixColors:[module_num:4|svn_version:\'Unknown\'|variable_revision_number:2|show_window:False|notes:\x5B\'Split HE and DAB into separate channels\'\x5D]
    Stain count:2
    ...
    Image name:LPR
    Stain:Custom
    Red absorbance:0.0320406
    Green absorbance:0.970037
    Blue absorbance:0.240834
...
IdentifyPrimaryObjects:[module_num:6|svn_version:\'Unknown\'|variable_revision_number:8|show_window:False|notes:\x5B"Identify DAB positive area\'s (yellow)"\x5D]
    Select the input image:LPR
    Name the primary objects to be identified:LPR_object
    Typical diameter of objects, in pixel units (Min,Max):8,5000000
...
MaskImage:[module_num:7|svn_version:\'Unknown\'|variable_revision_number:3|show_window:False|notes:\x5B\'Only analyse Nuclei (red) in the previously defined DAB tissue (yellow)\'\x5D]
    Select the input image:LPR
    Name the output image:LPR_object_mask
    Use objects or an image as a mask?:Objects
    Select object for mask:LPR_object
    Select image for mask:DAB
    Invert the mask?:No

MaskImage:[module_num:8|svn_version:\'Unknown\'|variable_revision_number:3|show_window:False|notes:\x5B\x5D]
    Select the input image:Hematoxylin
...
IdentifyPrimaryObjects:[module_num:9|svn_version:\'Unknown\'|variable_revision_number:8|show_window:False|notes:\x5B\'Find DAB positive nuclei (red) within masked image (green)\'\x5D]
    Select the input image:LPR_object_mask
    Name the primary objects to be identified:nuclei_LPR
    ...
    Suppress local maxima that are closer than this minimum allowed distance:7
    Speed up by using lower-resolution image to find local maxima?:Yes
    Name the outline image:nuclei_LPR_red
    Fill holes in identified objects?:Yes
    Automatically calculate size of smoothing filter?:Yes
    Automatically calculate minimum allowed distance between local maxima?:Yes
    Manual threshold:0.0
...
IdentifyPrimaryObjects:[module_num:10|svn_version:\'Unknown\'|variable_revision_number:8|show_window:False|notes:\x5B\'Find HE positive nuclei (blue) within masked image (green)\'\x5D]
...
OverlayOutlines:[module_num:11|svn_version:\'Unknown\'|variable_revision_number:2|show_window:False|notes:\x5B\'The next outline is ontop of the previous one.\'\x5D]
    ...
    Select outlines to display:nuclei_LPR_red
...
MeasureImageAreaOccupied:[module_num:13|svn_version:\'Unknown\'|variable_revision_number:3|show_window:False|notes:\x5B\'Measure HE nuclei (blue) and DAB area (yellow) and red) area\'\x5D]
    Hidden:4
    ...
    Retain a binary image of the object regions?:No
    Name the output binary image:Stain
    Select a binary image to measure:nuclei_LPR_red
    Measure the area occupied in a binary image, or in objects?:Binary Image
    Select objects to measure:None
    Retain a binary image of the object regions?:No
    Name the output binary image:Stain
